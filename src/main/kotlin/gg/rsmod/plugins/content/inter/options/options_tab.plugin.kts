...
bind_setting(child = ALL_SETTINGS_BUTTON_ID) {
    ...
    player.openInterface(interfaceId = ALL_SETTINGS_INTERFACE_ID, dest = InterfaceDestination.MAIN_SCREEN_ALTERNATE)
...
bind_setting(child = OptionsTab.ACCEPT_AID_BUTTON_ID) {
    player.toggleVarbit(OptionsTab.ACCEPT_AID_VARBIT)
}
...
bind_setting(child = 115) {
    player.setVarp(OSRSGameframe.SCREEN_BRIGHTNESS_VARP, player.getInteractingSlot() + 1)
}
...
on_button(OPTIONS_INTERFACE_ID, 37) {
    when (player.getInteractingSlot()) {
        2 -> player.toggleDisplayInterface(DisplayMode.RESIZABLE_CLASSIC)
        3 -> player.toggleDisplayInterface(DisplayMode.RESIZABLE_MODERN)
